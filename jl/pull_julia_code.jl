function get_files(; chapter_regex::Regex = r"include\{chapter")
    retval = String[]
    for line in readlines(joinpath("tex", "book.tex"))
...
if !isdir("output")
    ...
    if !isdir(joinpath("output", "chapter"))
        mkdir(joinpath("output", "chapter"))
    end
end

# Export all juliaconsole blocks
io = open("output/all_juliaconsole_blocks.jl", "w")
...
io = open("output/all_algorithm_blocks.jl", "w")
for filepath in get_files()
...
io = open("output/all_test_blocks.jl", "w")
